...
The data "economics" is included in the `ggplot2` package (see the documentation [provided here](https://ggplot2.tidyverse.org/reference/economics.html) for descriptions of the variables).  For this questions I'd like you to create two line graphs using `facet_grid` or `facet_wrap` showing the yearly mean of two different variables.  You may choose any two variables from the economics data (other than date) that you find interesting.  Each graph should display the yearly mean and be colored according to the president that year.  The code below provides the necessary presidential data.
...
Bonus (not required): Use `geom_ribbon` ([see an example](https://ggplot2.tidyverse.org/reference/geom_ribbon.html)) to add an uncertainty band using each year's minimum and maximum values.
...
economics2 <- separate(economics, date, into = c("year", "month", "day"), sep = "-")

econonomics_means <- economics2 %>% 
  group_by(year) %>% 
  summarise(mean_psavert = mean(psavert), mean_uempmed = mean(uempmed))


#president <- c(rep(NA,14), rep("Reagan", 8), rep("Bush1", 4), rep("Clinton", 8), rep("Bush2", 8), rep("Obama", 7))
econonomics_means$president <- presidents$president

econonomics_means2 <- gather(econonomics_means, var, val, 2:3)

p <- ggplot(data = econonomics_means2, aes(x = year, y = val, group = var, color = president)) + geom_line(size = 1.5) + facet_wrap(~ var)
p

econonomics_rib <- economics2 %>% 
  group_by(year) %>% 
  summarise(min_psavert = min(psavert), min_uempmed = min(uempmed), max_psavert = max(psavert), max_uempmed = max(uempmed))

econonomics_rib_min <- select(econonomics_rib, year, min_psavert, min_uempmed)
econonomics_rib_min2 <- gather(econonomics_rib_min, var, valmi, 2:3)

econonomics_rib_max <- select(econonomics_rib, year, max_psavert, max_uempmed)
econonomics_rib_max2 <- gather(econonomics_rib_max, var, valma, 2:3)

df <- data.frame(econonomics_means2, valmi = econonomics_rib_min2$valmi, valma = econonomics_rib_max2$valma)
p <- ggplot(data = df, aes(x = year, group = var)) + geom_ribbon(aes(ymin = valmi, ymax = valmi))+ facet_wrap(~ var)

+ geom_line(aes(y = val, group = var, color = president)) 
