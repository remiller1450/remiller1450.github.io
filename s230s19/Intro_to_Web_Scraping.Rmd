...
popNodes
```
There are four tables in this document. By inspecting the output of `popNodes`, we make a guess that we want the third table. (In other cases, trial and error may be required.) We select the third table by using double brackets, which are used to index list objects:
...
For the grosses, we note that the numbers are either prefaced by a dollar sign or end with a percentage, both of which will need to be removed. We will also have to remove the commas.  These tasks can be done using either `parse_number` or regular expressions.  An example of the latter is shown below:
...
So far the two examples we've looked at extract tables from a webpage.  Many other types of html nodes can be extracted using `rvest`.  The example below extracts information of the images on Box Office Mojo:
...
We can see that there are 10 images on this page.  We can also see that the page has Twitter and Facebook accounts.  There are a huge variety of nodes that we could extract, the next section provides another example of extracting a non-table node.

### 6. A NY Times Article Example

Sometimes we might want to extract nodes defined by CSS. To do this it can be helpful to view to html source code of a webpage for guidance.
...
The example we will look at is a [New York Times article](https://www.nytimes.com/interactive/2017/06/23/opinion/trumps-lies.html).  The goal for this example is transform the body of text into a tidy data.frame containing:
...
```{r}
lies_var <- lies_var[seq(2, length(lies_var), by = 3)]
```

At this point we've satisfied the goal of this example, next step might be a textual analysis.

```{r}
lies_clean <- data.frame(date = dates, lie = as.character(lies_var))
```
...
1. The web site [BikeRaceInfo](http://www.bikeraceinfo.com/tdf/tdfstats.html) has a table with data on past winners of the Tour de France. Create a cleaned-up data.frame of this data.

2. The web site [NY Times Best Sellers: Hardcover Fiction](http://www.nytimes.com/books/best-sellers/hardcover-fiction) contains a list of best-selling fiction books. Scrape the names of these top books. The list of books are tagged via `<h2>title</h2>`.

#### Option #2

3. The link https://www.auhouseprices.com/rent/list/NSW/2131/Ashfield/1/?sort=date&type=apartment&bmin=2&bmax=2 shows two-bedroom rental listings in Ashfield Australia.  Scrap this webpage to create a clean data.frame containing three variables: the rental's address, its price, and the month/year that the price is offered. 
